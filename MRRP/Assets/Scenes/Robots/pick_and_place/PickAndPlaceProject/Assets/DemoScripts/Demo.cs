...
public class Demo : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!m_HasPublished)
        {
            var plannerType = Type.GetType(k_TrajectoryPlannerType);
            dynamic publisher = GameObject.Find(k_PublisherName)
                .GetComponent(plannerType);
    ...
    void AddRosMessages()
    {
        ...
        {
            Type.GetType(typeFullName).GetMethod(k_RegisterMethodName).Invoke(null, null);
        }
    }
    ...
    void CreateTrajectoryPlannerPublisher()
    {
        ...
        var plannerType = Type.GetType(k_TrajectoryPlannerType);
        dynamic planner = publisher.AddComponent(plannerType);
    ...
    {
        // Runtime compilation using System.CodeDom is not supported in Unity's .NET Standard environment
        // Instead, force Unity to recompile scripts
        UnityEditor.Compilation.CompilationPipeline.RequestScriptCompilation();
        AssetDatabase.Refresh();
        
        // Note: The generated scripts will be compiled by Unity automatically
        // m_Assembly will not be set, so functionality relying on runtime assembly loading may not work
    }
